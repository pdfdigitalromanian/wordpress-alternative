...
import { starterDocument } from "~/lib/component-registry/starters";
...
export async function loader({ request, params }: Route.LoaderArgs) {
  ...
  const { data: pages, error: pagesError } = await supabase
    .from("pages")
    .select("id, slug, title, draft_updated_at, draft_document")
    .eq("site_id", siteId)
  ...
  const { data: livePages } = site.active_release_id ? await supabase.from("release_pages").select("page_id,title,slug,document").eq("release_id", site.active_release_id) : { data: [] };
  ...
  return { liveUrl, site, pages: (pages ?? []).map(page => ({ ...page, status: pageStatus(page, livePages ?? []) })), releases: releases ?? [], domains: domains ?? [] };
}
...
export async function action({ request, params }: Route.ActionArgs) {
  ...
  if (intent === "create-page") {
    ...
    const { slug, error: pathError } = validatePagePath(rawSlug);
    if (pathError) return { intent, error: pathError };
    ...
    const { error } = await supabase.from("pages").insert({
      ...
      title,
      created_by: user.id,
      draft_document: starterDocument("blank"),
    });
    if (error) {
  ...
  if (intent === "publish" || intent === "rollback") {
    return redirect(`/admin/sites/${siteId}/publishing`);
  }
...
export default function SiteDetail() {
  ...
  return (
    <main>
      ...
      <div className="notice-banner"><span className="notice-icon" aria-hidden="true">▤</span><div><strong>{pages.filter(page => page.status !== "Live").length} pages with unpublished changes</strong><p>Changes stay private until you publish. Publishing includes every page in this site.</p></div><a href={`/admin/sites/${site.id}/publishing`} className="btn-secondary">Review & publish →</a></div>
      ...
      <div className="overview-grid"><div>
        ...
        <section className="card" id="pages"><div className="section-heading"><div><span className="eyebrow">WEBSITE</span><h2>Your pages</h2></div><Link className="text-link" to={`/admin/sites/${site.id}/pages`}>Manage pages →</Link></div>
          {pages.length === 0 ? <div className="empty-state"><span aria-hidden="true">▤</span><h3>A blank canvas. Endless possibilities.</h3><p>Create your first page below. Add only what you need.</p></div> : <ul className="page-list">{pages.map((page) => <li key={page.id}><span className="page-icon" aria-hidden="true">▤</span><div><Link to={`/admin/sites/${site.id}/pages/${page.id}`}>{page.title}</Link><small>/{page.slug}</small></div><span className={`badge ${page.status === "Live" ? "badge-green" : "badge-amber"}`}>{page.status}</span><Link className="row-action" to={`/admin/sites/${site.id}/pages/${page.id}`}>Edit →</Link></li>)}</ul>}
